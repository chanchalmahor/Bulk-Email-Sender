...
SMTP_PASSWORD = "gkry"   # ← Replace with Gmail App Password
...
CONTACTS_CSV = "contacts.csv"   # ← your CSV file name
...
def load_contacts(path: str) -> list:
    ...
    with open(path, newline="", encoding="utf-8") as f:
        ...
        contacts = [row for row in reader if row.get("email")]
    print(f"✅ Loaded {len(contacts)} contacts from {path}")
    return contacts

HR_CONTACTS = load_contacts(CONTACTS_CSV)
